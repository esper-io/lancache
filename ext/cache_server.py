...
def root():
    ...
    return jsonify({"message": "Esper Local Cache Server"})
...
if __name__ == "__main__":
    try:
        logger.info("Starting integrated cache server on %s", socket.gethostname())
        prepare_saved_cache()
        read_remote_crc()
        read_saved_cache()
        ...
        context = (os.path.join(os.path.dirname(os.path.realpath(__file__)), "certs", "server.crt"),
