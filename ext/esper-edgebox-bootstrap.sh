...
#############################################
# make sure TOP_DIR is script’s own directory
PY_SCRIPT_URL="https://raw.githubusercontent.com/esper-io/lancache/master/ext/esper_edgebox.py"
UPDATED=0

echo -e "\n${GREEN}Checking Esper EdgeBox service version…${NC}"

# 1) Fetch remote __version__ without saving to disk
RAW_REMOTE_VER=$(
  curl -fsSL "$PY_SCRIPT_URL" \
    | grep -E '^__version__ *=' \
    | head -1 \
    | cut -d'"' -f2
)
echo -e "${YELLOW}Remote version: '${RAW_REMOTE_VER}'${NC}"

# 2) Read local __version__
LOCAL_VER=$(
  grep -E '^__version__ *=' "$TOP_DIR/esper_edgebox.py" \
    | head -1 \
    | cut -d'"' -f2
)
echo -e "${YELLOW} Local version: '${LOCAL_VER}'${NC}"

# 3) If different, pull the new script
if [[ -n "$RAW_REMOTE_VER" && "$RAW_REMOTE_VER" != "$LOCAL_VER" ]]; then
  echo -e "${GREEN}Updating esper_edgebox.py → v${RAW_REMOTE_VER}…${NC}"
