...
#############################################
# ensure TOP_DIR is script’s own directory
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TOP_DIR="$SCRIPT_DIR"

PY_SCRIPT_URL="https://raw.githubusercontent.com/esper-io/lancache/master/ext/esper_edgebox.py"
VER_URL="https://raw.githubusercontent.com/esper-io/lancache/master/ext/esper_edgebox.version"
UPDATED=0

echo -e "\n${GREEN}Checking Esper EdgeBox service version…${NC}"
# fetch, strip whitespace
RAW_REMOTE_VER=$($SUDO curl -fsSL "$VER_URL" 2>/dev/null || echo "")
REMOTE_VER=$(echo "$RAW_REMOTE_VER" | tr -d '\r\n' | xargs)
echo -e "${YELLOW}Remote version: '${REMOTE_VER}'${NC}"
